...
export default function CabPricingSection({
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem(`cabSelectedRules_${cabId}`);

    if (saved) {
      try {
        setSelectedRules(JSON.parse(saved));
      } catch {}
    }
  ...
  useEffect(() => {
    ...
    if (!data || !savedRules) return;

    const ids = data
      .filter(x => savedRules.includes(x.pricingRuleName))
  ...
  const finalTotal = (totalAmount ?? 0) + dutyExpenseAmount;
  ...
  useEffect(() => {
    ...
    if (totalKms <= 40) packageName = "40km";
    else if (totalKms <= 80) packageName = "80km";
    else packageName = "outstation";
  ...
  const has80 = selectedRules.some(id =>
    data?.find(x => x.pricingRuleId === id)
      ?.pricingRuleName.toLowerCase().includes("80km")
  );

  const hasOutstation = selectedRules.some(id =>
    data?.find(x => x.pricingRuleId === id)
      ?.pricingRuleName.toLowerCase().includes("outstation")
  );
          ...
          <tbody>
            ...
            {data.map((item) => {
              ...
              const disabled =
                ...
                (totalKms <= 40 && name.includes("outstation")) ||
